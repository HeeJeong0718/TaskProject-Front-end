<template>
 <v-container fluid>
    <v-row dense >
        <v-col v-for="support in supports" :key="support.name" :to="support.link"  cols="12" md="3" sm="6">
       <v-card>
           <v-toolbar color="orange" flat>
           <v-toolbar-title>{{support.name}}</v-toolbar-title>
           <v-spacer></v-spacer>      
           </v-toolbar>
           <v-card-text>rrr</v-card-text>
        </v-card>
        </v-col>
    </v-row>
    </v-container>
