...
<v-container>
  <v-app>
      ...
      <v-select outlined :items="department" v-model="department" 
      item-text="DEP_ID" item-value="DEP_NM" persistent-hint 
      return-object>
    ...
    insertform :function(){
       ...
       this.$http.post('http://localhost:8080/departlist',{},{
        ...
        for(var i=0; i<this.department.length; i++){
           this.department2.push({DEP_ID : ''});
           this.department2[i].dep_id = this.department[i].DEP_ID;
           console.log(this.department2);
        }
