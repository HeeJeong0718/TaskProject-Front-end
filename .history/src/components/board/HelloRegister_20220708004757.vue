  ...
  <v-app>
      ...
      <v-card-text>
        <v-form>
          ...
          <v-select :items="department" label="부서ID" v-model="department.dep_nm"   return-object>
            <option value="dep_nm">ddd</option>
          </v-select>
   ...
   methods:{
    insertform :function(){
       ...
       this.$http.post('http://localhost:8080/departlist',{},{
        ...
        this.departments = JSON.stringify(response.data.list.DEP_NM);
