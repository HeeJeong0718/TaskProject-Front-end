  ...
  <v-app>
      ...
      <v-card-text>
        <v-form>
          ...
          <v-select :items="department" label="부서ID" v-model="department"></v-select>
          <v-select :items="dep_nm" label="부서명" v-model="department_nm"></v-select>
            <select class="form-control">
          <option :key="i" :value="this.DEP_NM" v-for="(DEP_NM, i) in department">{{ DEP_NM }}</option>
        </select>
        </v-form>
      </v-card-text>
      <v-divider></v-divider>
   ...
   methods:{
    insertform :function(){
       ...
       this.$http.post('http://localhost:8080/departlist',{},{
        ...
        this.department = JSON.stringify(response.data.list.DEP_NM);
