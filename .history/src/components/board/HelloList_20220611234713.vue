    ...
    <div id="board" v-if="$store.state.userid">
      ...
      <v-text-field v-model="searchKeyword" append-icon="mdi-magnify" label="Search" single-line hide-details></v-text-field>
       ...
       <!--<table v-if="this.boards !='null'">
         ...
         <v-data-table :headers="headers" :items="boards"  :search="searchKeyword" @click:row="fnView(TITLE)">
           <template >
          ...
          <v-data-table :headers="headers" :items="boards">
            <template v-slot:body="{ boards }">
            <tbody>           
                <tr v-for="board in boards" :key="board.i"  :active="selectedItem == board.USERID" @click="selectedItem = board.USERID">
                    <td>{{ board.USERID }}</td>
                    <td>CONTENT</td>
...
export default {
    ...
    return {
          ...
          headers :[
            ...
            { text : "NAME" , align:"left", sortable : false, value :"USERNAME"},
