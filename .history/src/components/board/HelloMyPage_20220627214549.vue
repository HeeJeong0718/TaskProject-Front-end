  ...
  <v-app>
      ...
      <v-card-text>
        <v-form>
          ...
          <v-text-field label="userpass" v-model="myinfo.USERNAME"  prepend-icon="mdi-lock"></v-text-field>
   ...
   methods:{
      getMypage :function(){
       this.$http.post('http://localhost:8080/Mypage',{userid:this.$store.state.userid},{
        ...
        this.myinfo = JSON.stringify(response.data.list);
        this.dep = response.data.list.USERID;
       console.log("dep" + JSON.stringify(this.dep));
