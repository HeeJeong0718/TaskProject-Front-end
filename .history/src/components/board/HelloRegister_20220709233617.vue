...
<v-container>
  <v-app>
      ...
      <v-card-text>
        ...
        <v-select :items="leader" v-model="leader.dep_nm"   item-text="dep_id" item-value="dep_nm" return-object></v-select>
       </v-form>
      </v-card-text>
      <v-divider></v-divider>
      <v-card-actions>
   ...
   methods:{
    insertform :function(){
       ...
       this.$http.post('http://localhost:8080/departlist',{},{
        ...
        console.log(typeof(this.leader));
        
        for(var i=0; i<this.leader.size; i++){
           this.aa = this.leader.DEP_ID[i];
           console.log("aa::" +aa);
        }
         console.log("rs" + JSON.stringify(response.data.list[0].DEP_ID));
