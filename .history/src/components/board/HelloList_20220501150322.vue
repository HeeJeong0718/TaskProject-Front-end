      ...
      <span>vuex 에서 가져온 username : {{$store.state.userid}} </span> 
       <table>
           ...
           <tbody v-if="!searchKeyword">
            ...
            <tbody v-else>
              <tr :key="i" v-for="(searchboards, i) in searchboards"  @click="fnView(board.TITLE)">
                <td>{{searchboards.USERID}}2222</td>
                <td>{{searchboards.USERNAME}}</td>
                   <td>{{searchboards.TITLE}}</td>
                   <td>{{searchboards.CONTENT}}</td>
               </tr>
           </tbody>
       </table>
    </div>
    ...
    fnsearch(){
      alert("!!!!");
       this.$http.post('http://localhost:8080/testSearch',{content :this.searchKeyword},{
        ...
        this.searchboards = response.data;
