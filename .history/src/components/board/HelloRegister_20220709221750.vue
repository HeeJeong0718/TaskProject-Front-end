...
<v-container>
  <v-app>
      ...
      <v-select outlined :items="department" v-model="department" :hint="`${department.DEP_ID}`"
      item-text="department.DEP_ID" item-value="department.DEP_ID" persistent-hint return-object>
   ...
   methods:{
    insertform :function(){
       ...
       this.$http.post('http://localhost:8080/departlist',{},{
        ...
        this.department = JSON.stringify(response.data.list);
        this.departments = JSON.parse( this.department);
        this.department2 = JSON.stringify(response.data.list[0]);
        
         console.log("rs" + JSON.parse(response.data.list));
         console.log("this.boards" + JSON.stringify(this.aaa));
      })
    },
   }
}
