...
export default {
 data(){
     return{
           username :'',
           mem_id :this.$store.state.userid, //this붙이는거잊지말기
           title :'',
           content :'',
           due_date:'',
          modifyDetail :{}
 ...
 methods:{
     toBack(){
        ...
        write:function(){
        this.$http.post('/boardInsert',{ mem_id : this.mem_id ,title :this.title, content : this.content,due_date : this.due_date}, { //파라미터릂 여기에 줘야함 ->스프링 컨트롤러에 있는 vo랑똑같은이름써저야한다!!!
            method : 'POST',
            body : JSON.stringify({}),
            headers : {
