<template>
    <div id="board" v-if="$store.state.userid">
      ...
      <span>vuex 에서 가져온 username : {{$store.state.userid}} / {{$store.state.username}}</span> 
       <table>
           <thead>
               <tr>
                   <th>ID</th>
           ...
           <tbody>
              <tr :key="i" v-for="(board, i) in boards"  @click="fnView(boards.index)">
                <td>{{board.USERID}}</td>
    ...
    getList :function(){
     // const baseURL = 'http://localhost:8080/test2';
       this.$http.post('http://localhost:8080/testAll',{
        ...
        this.username = response.data.list.username;
         console.log("rs" + JSON.stringify(response.data.list));
         console.log("rsusername" + JSON.stringify(response.data.list.username));
      })
    },
