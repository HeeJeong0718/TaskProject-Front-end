<template>
  <div>
     <v-data-table :headers="headers" :items="department" item-key="title" :search="searchKeyword">
          <template slot="item" slot-scope="props">
            <tr >
                 {{props}}
              </tr>
          </template>
         </v-data-table>
       
  </div>
</template>

<script>
export default {
   data(){
    return {
      department : [],
        headers :[
            { text : "ID" , align:"left", sortable : false, value :"ID"},
            { text : "DEP_ID" , align:"left", sortable : true, value :"DEPARTMENT_ID"},
            { text : "DEP_NM" , align:"left", sortable : true, value :"DEPARTMENT_NM"},
           
          ]
    }
   },
    methods:{
    getList :function(){
       this.$http.post('http://localhost:8080/departlist',{userid:this.$store.state.userid, loginlevel : this.$store.state.loginlevel},{
            method : 'POST',
            body : JSON.stringify({userid : this.userid}),
           headers : {
         'Content-Type' : 'application/json',
         'Accept' : '*/*'
           } 
        }).then(response => {
          return response;
      }).then(response => {
        this.department = response.data.list;
         console.log("rs" + JSON.stringify(response.data.list));
         console.log("this.boards" + JSON.stringify(this.boards));
      })
    },
}}

</script>

<style>

</style>